...
この記事は クラスター Advent Calendat 2023 の 17日目の記事です。
...
cluster では Cluster Creator Kit と呼ばれるワールドやワールド内のアイテム、アバターのアクセサリーが作れるツールを Unity のパッケージとして配布しています。
本記事では、そんな Cluster Creator Kit とは無縁のはずの、サーバーっぽいものを作るための手法を幾つか紹介します。
明後日くらいには役に立つかも知れません。
...
リクエストとは、サーバーに対して「このデータをください」と要求することです。
ブラウザで任意の URL を開いた時も、裏ではブラウザが「このデータをください」ってやっています。
...
普段から目にすることのできる例としては、URL に `?` から始まる文字列としてが付与されているパラメーターが多いと思われます。

この記事を掲載している zenn というサイトには記事を検索するためのテキストフィールドがあると思いますが、そこで任意の文字列を入力して検索を実行した時、末尾に検索語句が含まれた URL がブラウザのアドレスバーに表示されると思います。
...
フォーム入力内容をサーバーに保存して欲しい場合などで使われています。
...
本記事では GCP の設定方法も掲載していますが、あくまでもとりあえずサーバーを動かすための設定ですのでご了承下さい。
...
cluster では Cluster Creator Kit と呼ばれるワールドやワールド内のアイテム、アバターのアクセサリーが作れるツールを Unity のパッケージとして配布しています。
本記事では、そんな Cluster Creator Kit とは無縁のはずの、サーバーっぽいものを作るための手法を幾つか紹介しました。
